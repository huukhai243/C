...
{ 
	...
	strcpy(sBuff, "BYE\r\n");
	ret = send(client, sBuff, strlen(sBuff), 0);
	memset(rBuff,  '\0', BUFF_SIZE);ret = recv(client, rBuff, BUFF_SIZE, 0);
	if (ret > 0) {
		rBuff[ret] = '\0';
		printf("Main: %s-->%s\n", sBuff, rBuff);
		if (strstr(rBuff, SUCCESS_LOGOUT) == 0)
	...
	ret = recv(client, rBuff, BUFF_SIZE, 0);
	if (ret > 0)
		printf("Stream test 2-1 fail!\n");

	strcpy(sBuff, "\r\n");
	ret = send(client, sBuff, strlen(sBuff), 0);
	ret = recv(client, rBuff, BUFF_SIZE, 0);
	if (ret <= 0)
		printf("Stream test 2-2 fail!\n");
	else {
		rBuff[ret] = '\0';
		printf("Main: %s-->%s\n", sBuff, rBuff);
	}
